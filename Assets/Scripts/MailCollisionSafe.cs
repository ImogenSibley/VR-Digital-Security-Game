...
public class MailCollisionSafe : MonoBehaviour
{
    ...
    {
        if (collision.gameObject.name == "SafeBin") { 
        ...
        else if (collision.gameObject.name == "SpamBin") // if mail object is sorted incorrectly
        {
            Debug.Log("collision detected");
            ContactPoint contact = collision.contacts[0];
            Quaternion rotation = Quaternion.FromToRotation(Vector3.up, contact.normal);
